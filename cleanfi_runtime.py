...
TEMP=Path(os.getenv('TEMP_DIR','./tmp'));TEMP.mkdir(parents=True,exist_ok=True);STATE=Path(os.getenv('STATE_FILE','./jobs.json'));jobs={};settings={};running={};sessions={};lock=asyncio.Lock()
...
async def save():
 async with lock:await asyncio.to_thread(sync_save)
def load():
 ...
 for j in jobs.values():
  for k,v in {'source':settings['source'],'target':settings['target'],'processed':[],'failed':[],'skipped':[],'failed_reasons':{},'progress_message_id':None,'started_at':None}.items():j.setdefault(k,v)
...
def current(m):return sessions.get(m.from_user.id)
def getjid(m):
 p=(m.text or '').split();return p[1] if len(p)>1 and p[1] in jobs else current(m)
async def tg(fn):
 while True:
  try:return await fn()
...
def progress(x):
 j=jobs[x];done=len(j['processed'])+len(j['failed'])+len(j['skipped']);total=j['total'];pct=int(done*100/total) if total else 100;w=14;n=int(w*pct/100);el=max(0,time.time()-(j.get('started_at') or time.time()));sp=done/(el/60) if done and el else 0;eta=((total-done)/sp*60) if sp else 0;bar='█'*n+'░'*(w-n)
...
async def pupdate(x,force=False):
 ...
 j['last_progress']=now
 try:
  if j.get('progress_message_id'):await tg(lambda:app.edit_message_text(j['owner'],j['progress_message_id'],progress(x)))
  else:j['progress_message_id']=(await tg(lambda:app.send_message(j['owner'],progress(x)))).id
  await save()
 except Exception:pass

@app.on_message(filters.private&filters.command('start'))
async def start(_,m):
 if ok(m):await m.reply_text('✦ CLEANFI\n\nAudiobook metadata cleaner & repacker.',reply_markup=main())
@app.on_message(filters.private&filters.command('help'))
async def helpcmd(_,m):
 if ok(m):await m.reply_text('✦ CLEANFI\n\n/range START END\n/source @channel\n/target @channel\n/meta artist="Name" genre="Romance" year=2026\n/cover JOBID\n/startjob JOBID\n/status JOBID\n/cancel JOBID\n/retry JOBID\n/failed JOBID\n/test MESSAGE_ID\n/jobs',reply_markup=main())
...
async def source(_,m):
 ...
 try:c=await resolve(p[1]);settings['source']=str(c.id);await save();await m.reply_text(await info('Source',settings['source']),reply_markup=main())
...
async def target(_,m):
 ...
 try:c=await resolve(p[1]);settings['target']=str(c.id);await save();await m.reply_text(await info('Target',settings['target']),reply_markup=main())
 except Exception as e:await m.reply_text(f'Could not set target: {e}')
@app.on_message(filters.private&filters.command('range'))
async def rangecmd(_,m):
 if not ok(m):return
 p=(m.text or '').split()
 if len(p)!=3 or not p[1].isdigit() or not p[2].isdigit() or int(p[1])>int(p[2]):return await m.reply_text('Usage: /range 1250 1300')
 if not settings.get('source') or not settings.get('target'):return await m.reply_text('Set /source and /target first.',reply_markup=main())
 a,b=map(int,p[1:]);x=uuid.uuid4().hex[:8];jobs[x]={'id':x,'owner':m.from_user.id,'start':a,'end':b,'total':b-a+1,'processed':[],'failed':[],'failed_reasons':{},'skipped':[],'status':'configured','cancel_requested':False,'meta':newmeta(),'source':settings['source'],'target':settings['target'],'created':time.time(),'started_at':None,'progress_message_id':None};sessions[m.from_user.id]=x;await save();await m.reply_text(summary(x),reply_markup=jobkb(x))
...
async def metacmd(_,m):
 ...
 await save();await m.reply_text(summary(x),reply_markup=jobkb(x))
@app.on_message(filters.private&filters.photo)
async def coverphoto(_,m):
 if not ok(m):return
 p=(m.caption or '').split();x=p[1] if len(p)==2 and p[0].lower()=='/cover' else current(m)
 if not x or x not in jobs:return
 d=TEMP/x;d.mkdir(exist_ok=True);path=d/'cover.jpg';await tg(lambda:m.download(file_name=str(path)));jobs[x]['meta']['cover_path']=str(path);await save();await m.reply_text(summary(x),reply_markup=jobkb(x))
@app.on_message(filters.private&filters.command('cover'))
async def covercmd(_,m):
 if ok(m):
  x=getjid(m)
  if x in jobs:await m.reply_text(f'Send image with caption /cover {x}')
@app.on_message(filters.private&filters.command('jobs'))
async def jobscmd(_,m):
 if ok(m):
  text='\n'.join(f"{x} — {j['status']} — {len(j['processed'])}/{j['total']}" for x,j in list(jobs.items())[-20:]) or 'No jobs.'
  await m.reply_text('✦ JOBS\n\n'+text,reply_markup=main())
@app.on_message(filters.private&filters.command('status'))
async def status(_,m):
 if not ok(m):return
 x=getjid(m)
 if x not in jobs:return await m.reply_text('Unknown job.')
 await m.reply_text(progress(x) if jobs[x]['status'] in {'running','queued','cancelling'} else summary(x),reply_markup=jobkb(x))
@app.on_message(filters.private&filters.command('failed'))
async def failed(_,m):
 if not ok(m):return
 x=getjid(m)
 if x not in jobs:return await m.reply_text('Unknown job.')
 j=jobs[x];text='\n'.join(f"{i} — {j['failed_reasons'].get(str(i),'unknown')}" for i in j['failed']) if j['failed'] else 'No failed files.';await m.reply_text('✦ FAILED\n\n'+text)
...
async def test(_,m):
 ...
 x=uuid.uuid4().hex[:8];i=int(p[1]);jobs[x]={'id':x,'owner':m.from_user.id,'start':i,'end':i,'total':1,'processed':[],'failed':[],'failed_reasons':{},'skipped':[],'status':'configured','cancel_requested':False,'meta':newmeta(),'source':settings['source'],'target':settings['target'],'created':time.time(),'started_at':None,'progress_message_id':None};sessions[m.from_user.id]=x;await save();await m.reply_text(summary(x),reply_markup=jobkb(x))
async def process(x,mid):
 ...
 name=getattr(media,'file_name',None) or f'message_{mid}.bin';ext=Path(name).suffix.lower();supported={'.mp3','.m4a','.mp4','.flac','.ogg','.opus','.wav','.aiff','.aif','.wma','.aac'}
 if ext not in supported:return 'skip','unsupported container '+(ext or 'unknown')
...
async def cb(_,q:CallbackQuery):
 ...
 if a=='m':
  ...
  if s=='source':return await q.message.reply_text(await info('Source',settings.get('source')))
  if s=='target':return await q.message.reply_text(await info('Target',settings.get('target')))
 ...
 if a=='clear':jobs[x]['meta']['cover_path']=None;await save();return await q.message.edit_text(summary(x),reply_markup=jobkb(x))
...
async def field_input(_,m):
 if not ok(m):return
 f=sessions.get((m.from_user.id,'field'));x=current(m)
 if not f or x not in jobs or (m.text or '').startswith('/'):return
 if f in jobs[x]['meta']:jobs[x]['meta'][f]=m.text;sessions.pop((m.from_user.id,'field'),None);await save();await m.reply_text(summary(x),reply_markup=jobkb(x))
load();app.run()
