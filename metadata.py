...
def read_original_title(path: str):
    ...
    try:
        ...
        if ext in {".m4a", ".mp4"}:
            f = MP4(p)
            return (f.get("\xa9nam") or [None])[0]
        if ext == ".flac":
            f = FLAC(p)
            return (f.get("title") or [None])[0]
        if ext == ".ogg":
            f = OggVorbis(p)
            return (f.get("title") or [None])[0]
        if ext == ".opus":
...
def _id3(path, title, artist, genre, year, album, album_artist, comment, cover):
    try:
        ...
        tags = ID3()
    tags.clear()
    # ID3v2.4 + UTF-8 is intentional: it preserves Hindi and other Unicode
    # text instead of producing mojibake when Telegram/player clients decode it.
    if title is not None:
        tags.add(TIT2(encoding=3, text=[_text(title)]))
    if artist:
        tags.add(TPE1(encoding=3, text=[_text(artist)]))
    if genre:
        tags.add(TCON(encoding=3, text=[_text(genre)]))
    if year:
        tags.add(TDRC(encoding=3, text=[_text(year)]))
    if album:
        tags.add(TALB(encoding=3, text=[_text(album)]))
    if album_artist:
        tags.add(TPE2(encoding=3, text=[_text(album_artist)]))
    if comment:
        tags.add(COMM(encoding=3, lang="eng", desc="", text=[_text(comment)]))
    if cover:
...
def _comments(f, title, artist, genre, year, album, album_artist, comment):
    ...
    if album:
        f["album"] = [_text(album)]
    if album_artist:
        f["albumartist"] = [_text(album_artist)]
    if comment:
        f["comment"] = [_text(comment)]
...
def clean_and_apply_metadata(input_path, output_path, *, title=None, artist=None, genre=None, year=None, cover=None, album=None, album_artist=None, comment=None):
    ...
    if ext == ".mp3":
        _id3(dst, title, artist, genre, year, album, album_artist, comment, cover_bytes)
        return
    ...
    if ext == ".flac":
        f = FLAC(dst)
        _comments(f, title, artist, genre, year, album, album_artist, comment)
        if cover_bytes:
            p = Picture(); p.type = 3; p.mime = _mime(cover_bytes); p.desc = "Cover"; p.data = cover_bytes
            f.add_picture(p)
        f.save()
        return
    ...
    if ext == ".wav":
        f = WAVE(dst)
        # WAVE metadata support differs between players; write ID3 only when supported.
        if f.tags is None:
            f.add_tags()
        _id3(dst, title, artist, genre, year, album, album_artist, comment, cover_bytes)
        return
    ...
    if ext == ".aac":
        # Raw ADTS AAC has no portable tag/artwork container. Preserve the stream.
        return
